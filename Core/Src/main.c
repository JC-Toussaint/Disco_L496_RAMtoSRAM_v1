...
/* Configure the peripherals common clocks */
  ...
  printf("WARNING : only the 16 least significant bits are copied to SRAM");
	...
	for(__IO uint32_t  uwIndex = 0; uwIndex < 16; uwIndex++)
	{
		__IO uint16_t uval = uwIndex;
		hal_status = HAL_SRAM_Write_16b(&hsram1, (uint32_t *)(SRAM_BANK_ADDR + WRITE_READ_ADDR + 2 * uwIndex), &uval, 1);
		OnError_Handler(hal_status != HAL_OK);
//		*(__IO uint16_t *)(SRAM_BANK_ADDR + WRITE_READ_ADDR + 2 * uwIndex) = uwIndex;
	}
	...
	for(__IO uint32_t  uwIndex = 0; uwIndex < 16; uwIndex++)
	{
		...
		hal_status = HAL_SRAM_Read_16b(&hsram1, (uint32_t *)(SRAM_BANK_ADDR + WRITE_READ_ADDR + 2 * uwIndex), &uval, 1);
		OnError_Handler(hal_status != HAL_OK);

//		__IO uint16_t uval = *(__IO uint16_t *)(SRAM_BANK_ADDR + WRITE_READ_ADDR + 2 * uwIndex);
//  		printf("%lu %u\n", uwIndex, uval);
		if (uval != uwIndex) Error_Handler();
	}
//	printf("\n");
#endif
...
#ifdef USE_SRAM
  if (HAL_DMA_Start_IT(&hdma_memtomem_dma1_channel1, (uint32_t)&aSRC_Buffer, (uint32_t)(SRAM_BANK_ADDR+WRITE_READ_ADDR), BUFFER_SIZE) != HAL_OK)
  {
    /* Transfer Error */
    Error_Handler();
  }
